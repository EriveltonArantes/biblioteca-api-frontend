    ...
    <div className="login-screen">
      <div className="login-institucional">
        ...
        <ul className="login-features">
          <li>✓ Controle completo de Autor</li>
...
function PainelAutor({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  const itensFiltrados = itens.filter(item => !busca || ["nome", "nacionalidade", "email", "dataNascimento", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  ...
  const carregar = () => fetch(apiBaseUrl + "/api/autors", { headers: authHeaders(token) })
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/autors/" + editId : apiBaseUrl + "/api/autors";
    fetch(url, {
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "nacionalidade", "email", "dataNascimento"].forEach(k => { f[k] = item[k] ?? ""; });
    [].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/autors/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
    ...
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["nome"] ?? "Autor")}</div>
            
            <div className="item-meta-grid">
            <div className="item-field"><b>nacionalidade</b><span>{item["nacionalidade"] ?? "—"}</span></div>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Autor</h3>
      ...
      <input type="text" value={form["nacionalidade"] ?? ""} onChange={e => setForm({...form, nacionalidade: e.target.value})} />
...
function PainelLivro({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  const [busca, setBusca] = React.useState("");
  const [pagina, setPagina] = React.useState(1);
  const [autorList, setAutorList] = React.useState([]);

  // Paginação real (achado real auditando como cliente exigente: lista sem
  // limite nenhum, "queria ver pelo menos os 10 primeiros"). Front-end só —
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["titulo", "isbn", "anoPublicacao", "genero", "autorId", "exemplares", "disponiveis", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);
  ...
  React.useEffect(() => {
    carregar();
    fetch(apiBaseUrl + "/api/autors", { headers: authHeaders(token) }).then(r => r.json()).then(setAutorList).catch(() => {});
  }, [token]);
  ...
  const editar = (item) => {
    const f = {};
    ["titulo", "isbn", "anoPublicacao", "genero", "autorId", "exemplares", "disponiveis"].forEach(k => { f[k] = item[k] ?? ""; });
    ["autor"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };
  ...
  return (
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-field"><b>genero</b><span>{item["genero"] ?? "—"}</span></div>
            <div className="item-field"><b>autorId</b><span>{item["autorId"] ?? "—"}</span></div>
            <div className="item-field"><b>exemplares</b><span>{item["exemplares"] ?? "—"}</span></div>
            <div className="item-field"><b>disponiveis</b><span>{item["disponiveis"] ?? "—"}</span></div>
            <div className="item-field"><b>autor</b><span>{item.autorId ? ((autorList.find(o => o.id === item.autorId) || {}).nome ?? ("#" + item.autorId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
              {role === "ADMIN" && <button className="btn btn-danger btn-sm" onClick={() => { if(window.confirm("Confirmar exclusão?")) apagar(item.id); }}>Apagar</button>}
      ...
      <select value={form["genero"] ?? "MASCULINO"} onChange={e => setForm({...form, genero: e.target.value})}>
        <option value="MASCULINO">MASCULINO</option>
        <option value="FEMININO">FEMININO</option>
        <option value="NAO_BINARIO">NAO_BINARIO</option>
        <option value="PREFIRO_NAO_INFORMAR">PREFIRO_NAO_INFORMAR</option>
      </select>
      <label className="field-label">autorId</label>
      <input type="number" value={form["autorId"] ?? ""} onChange={e => setForm({...form, autorId: e.target.value})} />
      ...
      <label className="field-label">disponiveis</label>
      <input type="number" value={form["disponiveis"] ?? ""} onChange={e => setForm({...form, disponiveis: e.target.value})} />
      <label className="field-label">autor</label>
      <select value={form["autor"] ?? ""} onChange={e => setForm({...form, autor: e.target.value})}>
        <option value="">Selecione...</option>
        {(autorList || []).map(o => (<option key={o.id} value={o.id}>{o.nome ?? ("#" + o.id)}</option>))}
      </select>
              <div className="modal-actions">
                <button type="submit" className="btn btn-primary" disabled={salvando}>{salvando ? "Salvando..." : "Salvar"}</button>
                <button type="button" className="btn btn-ghost" onClick={() => setModalAberto(false)} disabled={salvando}>Cancelar</button>
...
function PainelLeitor({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["nome", "cpf", "email", "telefone", "dataCadastro", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "cpf", "email", "telefone", "dataCadastro"].forEach(k => { f[k] = item[k] ?? ""; });
    [].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };
  ...
  return (
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-field"><b>dataCadastro</b><span>{item["dataCadastro"] ? new Date(item["dataCadastro"]).toLocaleString("pt-BR", {dateStyle:"short",timeStyle:"short"}) : "—"}</span></div>
      ...
      <label className="field-label">dataCadastro</label>
      <input type="datetime-local" value={form["dataCadastro"] ?? ""} onChange={e => setForm({...form, dataCadastro: e.target.value})} />
              <div className="modal-actions">
                <button type="submit" className="btn btn-primary" disabled={salvando}>{salvando ? "Salvando..." : "Salvar"}</button>
                <button type="button" className="btn btn-ghost" onClick={() => setModalAberto(false)} disabled={salvando}>Cancelar</button>
              </div>
...
function PainelEmprestimo({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["livroId", "leitorId", "dataEmprestimo", "dataDevolucaoPrevista", "dataDevolucaoReal", "status", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);
  ...
  const editar = (item) => {
    const f = {};
    ["livroId", "leitorId", "dataEmprestimo", "dataDevolucaoPrevista", "dataDevolucaoReal", "status"].forEach(k => { f[k] = item[k] ?? ""; });
  ...
  return (
    <div>
      ...
      {!modoKanban && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-field"><b>dataEmprestimo</b><span>{item["dataEmprestimo"] ? new Date(item["dataEmprestimo"]).toLocaleString("pt-BR", {dateStyle:"short",timeStyle:"short"}) : "—"}</span></div>
            <div className="item-field"><b>dataDevolucaoPrevista</b><span>{item["dataDevolucaoPrevista"] ? new Date(item["dataDevolucaoPrevista"]).toLocaleString("pt-BR", {dateStyle:"short",timeStyle:"short"}) : "—"}</span></div>
            <div className="item-field"><b>dataDevolucaoReal</b><span>{item["dataDevolucaoReal"] ? new Date(item["dataDevolucaoReal"]).toLocaleString("pt-BR", {dateStyle:"short",timeStyle:"short"}) : "—"}</span></div>
      ...
      <label className="field-label">dataEmprestimo</label>
      <input type="datetime-local" value={form["dataEmprestimo"] ?? ""} onChange={e => setForm({...form, dataEmprestimo: e.target.value})} />
      <label className="field-label">dataDevolucaoPrevista</label>
      <input type="datetime-local" value={form["dataDevolucaoPrevista"] ?? ""} onChange={e => setForm({...form, dataDevolucaoPrevista: e.target.value})} />
      <label className="field-label">dataDevolucaoReal</label>
      <input type="datetime-local" value={form["dataDevolucaoReal"] ?? ""} onChange={e => setForm({...form, dataDevolucaoReal: e.target.value})} />
...
export default function App() {
  ...
  const [aba, setAba] = React.useState("Autor");
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="sidebar-logo">Biblioteca<span className="dot">.</span></div>
        <button className={"nav-btn" + (aba === "Autor" ? " active" : "")} onClick={() => setAba("Autor")}><span className="nav-ico">🗂️</span>Autor</button>
      ...
      <main className="main">
        ...
        {aba === "Autor" && <PainelAutor token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
